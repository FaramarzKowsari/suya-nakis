...
const App = () => {
  ...
  const playVoice = (text) => {
    if ('speechSynthesis' in window) {
      ...
      // Select premium Google US English voice if available
  ...
  // --- 100-Phase Master Research Simulation ---
  const runDemo = () => {
    if (isDemoRunning) return;
    ...
    step(() => {}, 0, "Welcome. We are initiating a 100-phase master simulation of traditional Turkish Ebru art.");
    
    // Steps 1-40: Ground Pigments
    for (let i = 1; i <= 40; i++) {
      step(() => {
    ...
    step(() => {}, 500, "This advanced research project by Faramarz Kowsari introduces the profound intelligence of Turkish cultural heritage through mathematical fluid dynamics.");
    
    // Steps 41-80: Mathematical Interaction
    for (let i = 0; i < 40; i++) {
      step(() => applyTineLine(Math.random()*800, Math.random()*600, Math.random()*800, Math.random()*600, 30), 120);
    }

    step(() => {}, 500, "Every stroke honors ancient aesthetics while exploring modern computational frontiers. Turkish wisdom is now digitally preserved.");
    
    // Steps 81-100: Final Combing
    for (let i = 0; i < 20; i++) {
      step(() => applyCombMove(400, 50, 400, 550), 200);
    }

    step(() => setIsDemoRunning(false), 2000, "Simulation complete. Experience the digital elegance of Turkey.");
  };
    ...
    <div className="min-h-screen bg-stone-50 flex flex-col items-center p-4 md:p-8 font-sans text-stone-900 select-none overflow-x-hidden">
      ...
      {showPhilosophy && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] md:rounded-[4rem] p-6 md:p-12 max-w-3xl w-full shadow-2xl relative animate-in fade-in zoom-in duration-300">
            ...
            <h2 className="text-2xl md:text-4xl font-black text-emerald-800 mb-6 uppercase tracking-tighter italic leading-none">Philosophy of Ebru Art</h2>
            <p className="text-stone-600 leading-relaxed text-sm md:text-lg mb-8 italic text-justify">{philosophyText}</p>
            <button onClick={() => playVoice(philosophyText)} className="flex items-center gap-3 bg-emerald-700 text-white px-8 py-4 rounded-2xl font-black uppercase tracking-widest hover:bg-emerald-800 active:scale-95 transition-all shadow-xl">
      ...
      <div className="max-w-6xl w-full flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div className="text-center md:text-left">
          ...
          <p className="text-stone-400 font-bold text-[8px] md:text-xs tracking-[0.3em] mt-1 uppercase">Advanced Virtual Ebru Lab / <span className="text-emerald-600 font-black tracking-widest">Research Edition</span></p>
        </div>
        <div className="flex flex-wrap justify-center gap-2">
          <button onClick={runDemo} disabled={isDemoRunning} className={`group p-3 md:p-4 md:px-8 bg-stone-900 text-white rounded-xl md:rounded-2xl shadow-xl transition-all active:scale-95 ${isDemoRunning ? 'opacity-50' : 'hover:bg-emerald-950'}`}>
            <div className="flex items-center gap-2 md:gap-3"><PlayCircle size={24} className={isDemoRunning ? '' : 'text-emerald-400 animate-pulse'} /><p className="text-[10px] md:text-[12px] font-black uppercase tracking-widest leading-none">Master Demo</p></div>
          </button>
          ...
          <button onClick={() => { const link = document.createElement('a'); link.download = 'ebru-masterpiece.png'; link.href = canvasRef.current.toDataURL(); link.click(); }} className="p-3 md:p-4 bg-emerald-700 text-white rounded-xl md:rounded-2xl shadow-lg flex items-center gap-2 font-black uppercase text-[10px]">Export</button>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-4 md:gap-8 max-w-7xl w-full">
        ...
        <div className="lg:w-80 space-y-4 md:space-y-6">
          <div className="bg-white p-6 md:p-10 rounded-2xl md:rounded-[3.5rem] border border-stone-100 shadow-xl">
             <div className="flex items-center gap-4 mb-6"><div className="bg-emerald-100 p-3 md:p-4 rounded-xl text-emerald-700"><HelpCircle size={32} /></div><p className="font-black text-sm uppercase tracking-widest text-stone-900 leading-none">Research</p></div>
             <p className="text-[11px] md:text-[12px] leading-relaxed text-stone-500 text-justify italic font-medium">
               A bridge between traditional Turkish wisdom and high-fidelity computing. Developed as part of Faramarz Kowsari's cultural research.
             </p>
          </div>
          <div className="bg-emerald-900 text-white p-6 md:p-10 rounded-2xl md:rounded-[3.5rem] shadow-xl relative overflow-hidden group">
            ...
            <div className="mt-8 space-y-3">
              ...
              <a href="https://play.google.com/store/search?q=Faramarz_Kowsari&c=books" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-[10px] font-black uppercase text-emerald-200 hover:text-white transition-all underline decoration-emerald-500/50">
                <Library size={14} /> Google Play Books
              </a>
            </div>
          </div>
        </div>

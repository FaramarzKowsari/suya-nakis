...
import { Palette, MousePointer2, Wand2, Download, Info, Trash2, Volume2, HelpCircle, PlayCircle, ExternalLink, Linkedin, BookOpen, Layers } from 'lucide-react';

// --- Ebru Physics Engine Constants ---
const CANVAS_WIDTH = 800;
const CANVAS_HEIGHT = 600;
const VERTEX_COUNT = 500; // 500 points for ultra-high fidelity
    ...
    <div className="min-h-screen bg-stone-50 flex flex-col items-center p-4 md:p-8 font-sans text-stone-900">
      <div className="max-w-6xl w-full flex flex-col md:flex-row justify-between items-center mb-10 gap-6">
        <div className="text-center md:text-left">
          ...
          <p className="text-stone-400 font-bold text-[10px] md:text-xs tracking-[0.4em] mt-2 uppercase">100-Phase Master Simulation / <span className="text-stone-600">100 Aşamalı Ustalık Simülasyonu</span></p>
        </div>
        <div className="flex flex-wrap justify-center gap-3">
          <button onClick={run100StepDemo} disabled={isDemoRunning} className={`group relative overflow-hidden p-4 md:px-8 bg-stone-900 text-white rounded-2xl shadow-2xl transition-all active:scale-95 ${isDemoRunning ? 'opacity-50' : 'hover:bg-emerald-950 hover:shadow-emerald-200'}`}>
            <div className="flex items-center gap-3"><PlayCircle size={24} className={isDemoRunning ? 'text-stone-500' : 'text-emerald-400 animate-pulse'} /><div className="text-left"><p className="text-[12px] font-black uppercase tracking-widest leading-none">Run 100-Step Demo</p><p className="text-[8px] font-bold text-stone-400 mt-1 uppercase">100 Adımlık Demoyu Başlat</p></div></div>
          </button>
      ...
      <div className="flex flex-col lg:flex-row gap-8 max-w-7xl w-full">
        ...
        <div className={`relative flex-grow shadow-2xl rounded-[5rem] overflow-hidden border-[24px] border-white bg-white outline outline-1 outline-stone-200 ${isDemoRunning ? 'cursor-wait' : ''}`}>
          ...
          {isDemoRunning && <div className="absolute inset-0 bg-black/10 backdrop-blur-[1px] flex items-center justify-center pointer-events-none"><div className="bg-white/90 px-10 py-5 rounded-full font-black text-xs shadow-2xl tracking-[0.3em] text-stone-900 border border-white animate-bounce">100-STEP MASTERCLASS IN PROGRESS</div></div>}
          <div className="absolute bottom-10 left-1/2 -translate-x-1/2 w-[85%] flex flex-col md:flex-row items-center gap-10 bg-stone-900/95 backdrop-blur-3xl p-8 rounded-[3.5rem] text-white shadow-2xl border border-white/10">
             ...
             <div className="flex-grow w-full flex flex-col gap-3"><div className="flex justify-between text-[10px] font-black opacity-40 uppercase tracking-widest"><span>500-Vertex Fluid Precision</span><span>Brush Size: {brushSize}px</span></div><input type="range" min="10" max="250" value={brushSize} onChange={(e) => setBrushSize(parseInt(e.target.value))} className="w-full h-1 bg-white/10 rounded-lg appearance-none cursor-pointer accent-emerald-400" /></div>
          </div>
        </div>
        <div className="lg:w-96 space-y-6">
          <div className="bg-white p-10 rounded-[4rem] border border-stone-100 shadow-xl">
             ...
             <div className="space-y-8"><Step n="40" en="Battal Base" tr="40 adımda zemin boyama." /><Step n="30" en="Linear Flow" tr="30 adımda akış ve doku." /><Step n="20" en="Ornament" tr="20 adımda süsleme." /><Step n="10" en="Finishing" tr="10 adımda sonuçlandırma." /></div>
          </div>
          <div className="bg-white p-10 rounded-[4rem] border border-emerald-50 shadow-xl space-y-6">
             ...
             <div className="space-y-4">
                ...
                <div className="p-5 bg-emerald-900 text-white rounded-[2.5rem] shadow-lg relative overflow-hidden group">
                    ...
                    <p className="text-[9px] mt-2 italic opacity-70 border-t border-white/10 pt-2 text-justify">Faramarz Kowsari'nin Türkiye, sanat, kültür, dil ve gelenekler hakkındaki kitapları için:</p>
...
const ToolBtn = ({ active, onClick, icon, en, tr }) => (
  <button onClick={onClick} className={`p-8 rounded-[3rem] flex flex-col items-center gap-2 transition-all ${active ? 'bg-stone-900 text-white shadow-2xl scale-110' : 'text-stone-300 hover:bg-stone-50 hover:text-stone-900'}`}><div className="flex flex-col text-center leading-tight">{React.cloneElement(icon, { size: 36 })}<span className="text-[12px] font-black mt-1">{en}</span><span className="text-[9px] font-bold opacity-40">{tr}</span></div></button>
);
const Step = ({ n, en, tr }) => (

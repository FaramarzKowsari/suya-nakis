...
const VERTEX_COUNT = 400; // Optimized for performance and stability

const App = () => {
  const canvasRef = useRef(null);
  const dropsRef = useRef([]); // High-speed storage to prevent React re-render lag
  ...
  const applyTineLine = (x1, y1, x2, y2, forceVal = 30) => {
    const dx = x2 - x1;
    const dy = y2 - y1;
  ...
  const applyCombMove = (x1, y1, x2, y2, combSpacing = 35, combForce = 15) => {
    const dx = x2 - x1;
    const dy = y2 - y1;
    const dist = Math.sqrt(dx * dx + dy * dy);
    if (dist < 0.5) return;
  ...
  // --- Rendering Loop (Fixed White Screen Issue) ---
  const draw = () => {
  ...
  const run100StepDemo = () => {
    ...
    step(() => setIsDemoRunning(false), 2000, "نمایش با موفقیت به پایان رسید.");
  };
    ...
    <div className="min-h-screen bg-stone-50 flex flex-col items-center p-4 md:p-8 font-sans text-stone-900 select-none">
      ...
      <div className="max-w-6xl w-full flex flex-col md:flex-row justify-between items-center mb-10 gap-6">
        <div className="text-center md:text-left">
          ...
          <p className="text-stone-400 font-bold text-[10px] md:text-xs tracking-[0.4em] mt-2 uppercase underline decoration-emerald-500/30">Stable Physics Engine v2.0</p>
        </div>
        <div className="flex flex-wrap justify-center gap-3">
          ...
          <button onClick={() => { const link = document.createElement('a'); link.download = 'my-ebru-art.png'; link.href = canvasRef.current.toDataURL(); link.click(); }} className="p-4 bg-emerald-700 text-white rounded-2xl shadow-lg hover:bg-emerald-800 transition-all flex items-center gap-2"><Download size={24} /></button>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8 max-w-7xl w-full h-full">
        ...
        {/* Interactive Canvas */}
        <div className={`relative flex-grow shadow-2xl rounded-[5rem] overflow-hidden border-[20px] border-white bg-white outline outline-1 outline-stone-200`}>
          <canvas 
            ref={canvasRef} 
            width={CANVAS_WIDTH} 
          ...
          <div className="absolute bottom-10 left-1/2 -translate-x-1/2 w-[85%] flex flex-col md:flex-row items-center gap-10 bg-stone-900/95 backdrop-blur-3xl p-8 rounded-[3.5rem] text-white shadow-2xl">
             <div className="flex gap-4">
               {['#1D3557', '#40E0D0', '#FFB703', '#E63946', '#F1FAEE'].map(c => (
                 <button key={c} onClick={() => setCurrentColor(c)} className={`w-10 h-10 rounded-full border-2 transition-all ${currentColor === c ? 'border-white scale-125 ring-4 ring-white/10' : 'border-transparent opacity-50'}`} style={{ backgroundColor: c }} />
               ))}
             </div>
        ...
        <div className="lg:w-80 space-y-6">
          <div className="bg-white p-8 rounded-[3rem] border border-stone-100 shadow-xl">
             ...
             <p className="text-[11px] leading-relaxed text-stone-500 text-justify italic">
               هنر ابرو، رقص رنگ‌ها بر بستر آب است. این نسخه با موتور فیزیک سیالات بهینه شده تا تجربه‌ای روان و بدون وقفه داشته باشید.
             </p>
          </div>
